...
class ColorCube {
  var A1, B1, C1, pA1, pB1, pC1, A2, D2, M2, N2, pA2, pB2, pD2, pE2, pM2, pN2, A3, B3, C3, pA3, pB3, pC3;
  int k;
  int size;
  ...
  canvasCoord(var x, var y) {
    return [
      (x + size ~/ 2).round().toInt(),
      (size ~/ 2 - y).round().toInt()
    ];
  }
  ...
  void drawColorTriangle(ImageData imgData, List p1, List p2, List p3, List A, List B, List C, var ymax, bool orientation) {
    var delta, compare;
    if (orientation) {
      delta = -1;
    ...
    while (compare(y, ymax)) {
      for (var x = x1; x <= x2; x++) {
        ...
        color = colorAtPoint(p, p1, p2, p3, A[0], A[1], A[2], B[0], B[1], B[2], C[0], C[1], C[2]);
  ...
  colorAtPoint(List p, List p1, List p2, List p3, var r1, var g1, var b1, var r2, var g2, var b2, var r3, var g3, var b3) {
